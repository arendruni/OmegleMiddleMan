...
io.on("connection", function (socket) {
	...
	function makeConnection(args, reconnect) {
		...
		function handleCaptcha(code) {
			...
			if (proxyEnabled && proxyInfo) {
				...
				socket.emit(
					"proxyMessage",
					"Server sent a capcha, seaching for a new proxy...",
					args,
				);
			...
			om.start(function (err) {
				if (err) {
					if (proxyEnabled && proxyInfo) {
						...
						socket.emit(
							"proxyMessage",
							"Broken proxy, seaching for a new proxy...",
							args,
						);
	...
	function tryFindNewProxy(callback) {
		...
		request("http://gimmeproxy.com/api/getProxy?get=true&protocol=http", function (
			err,
			res,
			body,
		) {
