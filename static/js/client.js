...
var omegleSettings = omegleSettings || {
	...
	defaultMessage:
		"Please make a copy of static/js/settings_example.js, and call it static/js/settings.js You can change the settings to fit your needs",
	reroll: true,
	moderated: true,
	spy: false,
	ask: false,
    ...
    */
	...
	pMap.socket.on("omegleNewChallenge", function (args, code) {
		...
		for (var key in pMap.pains) {
			...
			if (p.painID == args.painID) {
				// Add the image
				p.addTextLine(
					"You need to do recaptcha and it is currently broken. Jump on the website, do the recaptcha, then come back here.",
				);
	...
	pMap.socket.on("omegleBanned", function (args) {
		...
		for (var key in pMap.pains) {
			...
			if (p.painID == args.painID) {
				...
				// Log the problem
				p.addTextLine(
					'<b><font color="red">You are banned.</font></b> Switching to unmoderated...',
				);
	...
	pMap.socket.on("omegleConnected", function (client_id, peerID) {
		...
		if (p) {
			...
			p.autoDisconnect(
				client_id,
				(omegleSettings.maxWaitType || 15) * 1000,
				(omegleSettings.maxWaitMessage || 45) * 1000,
			);
	...
	pMap.socket.on("omegleCommonLikes", function (client_id, commonLikes) {
		...
		if (p) {
			...
			if (
				omegleSettings.maxCommonInterests != null &&
				totalLikes > omegleSettings.maxCommonInterests
			) {
				// Disconnect
		...
		if (p) {
			if (!p.wontAutoDisconnect) {
				...
				for (var i = 0; i < autoBlackHoleList.length; ++i) {
					...
					if (flags.exact) {
						...
						try {
							...
							var errorMessage =
								"Check your BlackHole Regex: " + htmlEntities(e.message);
							p.addTextLine(
								'<font color="red">Meta:</font> ' + errorMessage,
								errorMessage,
								"Meta",
							);
						}
					...
					if (doBlackhole) {
						...
						// Add the message
						p.addTextLine(
							'<font color="red">Stranger:</font> ' + htmlEntities(msg),
							msg,
							"Stranger",
						);
				...
				if (omegleSettings.agressiveBotIgnore) {
					if (
						!p.hasTyped &&
						p.ignoreBots.is(":checked") &&
						p.getTimeConnected() < 3 &&
						!p.hasSpoken
					) {
						p.dontAutoSend = true;
						...
						setTimeout(function () {
							if (p.getClientId() == client_id) {
								pMap.doDisconnect(
									client_id,
									null,
									"Disconnected: Bot or phone user.",
								);
							}
						}, 1000);
						return;
	...
	pMap.socket.on("omegleSpyMessage", function (client_id, spy, msg) {
		...
		if (p) {
			...
			// Add the message
			p.addTextLine(
				'<font color="red">' + htmlEntities(spy) + ":</font> " + htmlEntities(msg),
				msg,
				"Spy",
			);
...
pain.prototype.createConnection = function () {
	...
	if (this.college.is(":checked")) {
		...
		if (omegleSettings.college && omegleSettings.college_auth) {
			...
			// Tell the user
			this.addTextLine(
				"You need to add <b>college</b> and <b>college_auth</b> to your omegleSettings in static/js/settings.js",
			);
		}
	}
		...
		setTimeout(function () {
			...
			if (p.getClientId() == client_id && !p.dontAutoSend) {
				...
				var highlight = p.addTextLine(
					'<font color="blue">Auto:</font> ' + htmlEntities(txt),
					txt,
					"Me",
				);
...
pain.prototype.autoDisconnect = function (client_id, delay, delay2) {
	...
	if (p.ignoreBots.is(":checked")) {
		...
		setTimeout(function () {
			// Check if the same client is connected
			if (
				p.getClientId() == client_id &&
				!p.hasTyped &&
				!p.hasSpoken &&
				p.ignoreBots.is(":checked") &&
				!p.wontAutoDisconnect
			) {
				// Disconnect
...
pain.prototype.generateLogOption = function () {
	...
	if (
		Math.ceil(this.field.scrollTop() + this.field.innerHeight()) >=
		Math.floor(this.field.prop("scrollHeight"))
	) {
		shouldScroll = true;
	}
			...
			if (tick && tick2) {
				...
				if (tick.is(":checked")) {
					...
					if (tick2.is(":checked") && !nameOverride) {
						...
						// Add it to our log
						var highlight = p.addTextLine(
							'<font color="blue">Broadcasted:</font> ' + htmlEntities(msg),
							msg,
							"Broadcasted",
						);
...
pain.prototype.sendCurrentMessage = function () {
	...
	if (txt != "") {
		// Add it to our log
		var highlight = this.addTextLine(
			'<font color="blue">You:</font> ' + htmlEntities(txt),
			txt,
			"Me",
		);
...
helperPain.prototype.setup = function () {
	...
	}).appendTo($("<tr>").appendTo(this.container));
						...
						click: function () {
							...
							if (myMessage.length > 0) {
								...
								setTimeout(function () {
									...
									if (
										Math.ceil(
											pain.field.scrollTop() + pain.field.innerHeight(),
										) >= Math.floor(pain.field.prop("scrollHeight"))
									) {
										shouldScroll = true;
									}
